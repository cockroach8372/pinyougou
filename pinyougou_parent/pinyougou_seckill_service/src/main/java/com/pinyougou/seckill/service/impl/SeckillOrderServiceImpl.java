...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId, String userId) {
		...
		if (seckillGoods.getStockCount()<=0){
			throw new RuntimeException("商品已抢购空");
		}
		seckillGoods.setStockCount(seckillGoods.getStockCount()-1);//库存-1；
		redisTemplate.boundHashOps("seckillGoods").put(seckillId,seckillGoods);//重新存到redis
		if (seckillGoods.getStockCount()==0){//-1后库存玩0，秒光
